using ContosoRetail.SharedKernel.DataAccess.DataLoader.Helpers;
...
namespace ContosoRetail.SharedKernel.DataAccess.DataLoader
{
    public class DataSourceLoader
    {
        ...
        static IQueryable<R> ExecQuery<S, R>(Func<IQueryable<S>, IQueryable<R>> query, IQueryable<S> source, ContosoRetail.SharedKernel.DataAccess.DataLoader.DataSourceLoadOptionsBase options)
        {
            var result = query(source);

#if DEBUG
            if (options.UseQueryableOnce)
                result = new ContosoRetail.SharedKernel.DataAccess.DataLoader.QueryableOnce<R>(result);
